...
Процедура ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере(ИсточникДанных, _ПравилаГруппировки)
	...
	// -------------------------------------------------------
	// Предусматривает добавления строк в отчет имеющий строки
	СсылкаДобавить					= "ДобавитьСтрокуЗначок";
	_ПрефиксУровня0					= "П10000";
	_ОписаниеКрайнейстрокиВОтчете	= НайтиКрайнююОбластьДанных(_ТабличныйДокументРегОтчета.Области, СсылкаДобавить, _ПрефиксУровня0, 0);
	_ИндексКрайнегоЗначенияУровня0	= ?(_ОписаниеКрайнейстрокиВОтчете.Индекс = 1, _ОписаниеКрайнейстрокиВОтчете.Индекс, _ОписаниеКрайнейстрокиВОтчете.Индекс + 1);
	// -------------------------------------------------------
	
	_ПропуститьСтрокиПоУмолчанию = Новый Соответствие;
	_ПропуститьСтрокиПоУмолчанию.Вставить( СтрШаблон("П10000_%1", 	_ИндексКрайнегоЗначенияУровня0), Истина);
	_ПропуститьСтрокиПоУмолчанию.Вставить( СтрШаблон("П11000_%1_1", _ИндексКрайнегоЗначенияУровня0), Истина);
	_ПропуститьСтрокиПоУмолчанию.Вставить( СтрШаблон("П12000_%1_1", _ИндексКрайнегоЗначенияУровня0), Истина);	
	...
	Для Каждого _СтрокаТЗ Из ИсточникДанных Цикл
		...
		Пока ИтераторЗаписейТЗПоЗначениюУровеня0 < _мВсеЗаписиТЗПоЗначениюУровеня0.Количество() Цикл
			...
			Пока ИтераторУровняN < _ПравилаГруппировки.Количество() Цикл
				...
				ОписаниеДобавления = _ДобавитьСтрокуВТабличныйДокумент(_ТабличныйДокументРегОтчета, ИтераторУровняN, _ПравилаГруппировки,,_ПропуститьСтрокиПоУмолчанию);
				// <---------------
...
&НаСервере
Функция _ДобавитьСтрокуВТабличныйДокумент(ТабДок, Уровень, _ПравилаГруппировки, _СтрокаМакетаДобавленаРекурсивно = Ложь, _ПропуститьСтрокиПоУмолчанию = Неопределено)

#Область ИнициализацияПеременных_ДобавитьСтрокуВТабличныйДокумент	
	...
	_ПрефиксУровня	= Новый Соответствие;
	_ПрефиксУровня.Вставить(0, "П10000");
	_ПрефиксУровня.Вставить(1, "П11000");
	_ПрефиксУровня.Вставить(2, "П12000");
	
	_ОписаниеУровней	= Новый Соответствие;
	Для Каждого _Правило Из _ПравилаГруппировки Цикл
		_ОписаниеУровней.Вставить(_Правило.Ключ);
	КонецЦикла;
	
#КонецОбласти	
	
#Область НайтиКрайнююСтрокуВМакете

	ИтераторУровня	= 0;	
	Пока ИтераторУровня < _ОписаниеУровней.Количество() Цикл
		
		//Результат.Индекс
		//Результат.ИРодителя
		//Результат.ИмяОбласти
		
		_ОписаниеУровней[ИтераторУровня]	= НайтиКрайнююОбластьДанных(ТабДокОбласти, СсылкаДобавить, _ПрефиксУровня.Получить(Уровень), Уровень);
		ИтераторУровня 						= ИтераторУровня + 1;
	КонецЦикла;
	
#КонецОбласти
	
	ОписаниеУровня0			= _ОписаниеУровней.Получить(0);
	ОписаниеТекущегоУровня	= _ОписаниеУровней.Получить(Уровень);
	ИмяКнопкиТекОбласти		= ОписаниеТекущегоУровня.ИмяОбласти;
...
#Область ФормированиеРезультата
	...
	Результат.Префикс   		= _ПрефиксУровня.Получить(Уровень);
	Результат.ИндексНового 		= ОписаниеТекущегоУровня.Индекс;
	Результат.Имя				= СтрЗаменить(ИмяКнопкиТекОбласти, СсылкаДобавить, "");
	Результат.Номер				= Число(СтрЗаменить(Результат.Префикс, "П", ""));
	Результат.НаходитсяВСтроке 	= ОписаниеТекущегоУровня.ИРодителя;
...
Если _ПропуститьСтрокиПоУмолчанию.Количество() И _ПропуститьСтрокиПоУмолчанию.Получить(Результат.Имя) <> Неопределено Тогда
	_ПропуститьСтрокиПоУмолчанию.Удалить(Результат.Имя);
	Возврат Результат;
ИначеЕсли _ПропуститьСтрокиПоУмолчанию.Количество() = 0 И Уровень = 0 Тогда
	// Первую не добавляем в пропуск
	//_ПропуститьСтрокиПоУмолчанию.Вставить("П10000_1", 	Истина);
	_ПропуститьСтрокиПоУмолчанию.Вставить( СтрШаблон("П11000_%1_1", ОписаниеТекущегоУровня.Индекс + 1), Истина);
	_ПропуститьСтрокиПоУмолчанию.Вставить( СтрШаблон("П12000_%1_1", ОписаниеТекущегоУровня.Индекс + 1), Истина);
КонецЕсли;

#КонецОбласти

#Область ДобавитьНовуюСтрокуВТабДокумент

Если НЕ _СтрокаМакетаДобавленаРекурсивно Тогда
	_СтрокаМакетаДобавленаРекурсивно = Истина;
	...
	Результат = _ДобавитьСтрокуВТабличныйДокумент(ТабДок, Уровень, _ПравилаГруппировки, _СтрокаМакетаДобавленаРекурсивно, _ПропуститьСтрокиПоУмолчанию);
КонецЕсли;	
...
Функция _ФормироватьИмяКнопкиТекОбласти(Уровень, ИмяКнопки, ПрефиксУровня, Уровня0Индекс, УровняNИндекс)

	ИмяКнопкиТекОбласти = Неопределено;
	
	Если Уровень = 0 Тогда
		ИмяКнопкиТекОбласти = СтрШаблон("%1%2_%3", 	  ИмяКнопки, ПрефиксУровня, Уровня0Индекс);
	Иначе		
		ИмяКнопкиТекОбласти	= СтрШаблон("%1%2_%3_%4", ИмяКнопки, ПрефиксУровня, Уровня0Индекс, УровняNИндекс);
	КонецЕсли;
	
	Возврат ИмяКнопкиТекОбласти;
	
КонецФункции

// Если Это новая строка тогда возврат Неопределено!
&НаСервере
Функция НайтиКрайнююОбластьДанных(ТабДокОбласти, ИмяКнопки, ПрефиксУровня, УровняN = Неопределено)
	
	Результат 		= Новый Структура;
	
	Результат.Вставить("Индекс");
	Результат.Вставить("ИРодителя");
	Результат.Вставить("ИмяОбласти");
	
	НайденаОбласть 	= Ложь;
	ИндексСтрокиА	= 1;	
	
	Пока ИндексСтрокиА < ТабДокОбласти.Количество() Цикл
		
		ИндексСтрокиБ	= 1;
		Пока ИндексСтрокиБ < ТабДокОбласти.Количество() Цикл
			
			Если УровняN = 0 Тогда
				ИмяИскомойОбласти	= СтрШаблон("%1%2_%3", ИмяКнопки, ПрефиксУровня, ИндексСтрокиБ);
			Иначе
				ИмяИскомойОбласти	= СтрШаблон("%1%2_%3_%4", ИмяКнопки, ПрефиксУровня, ИндексСтрокиА, ИндексСтрокиБ);
			КонецЕсли;
			НайденаОбласть			= ТабДокОбласти.Найти(ИмяИскомойОбласти);
			
			Если НайденаОбласть <> Неопределено Тогда
				
				Результат.Индекс		= ИндексСтрокиБ;
				Результат.ИРодителя		= ?(УровняN = 0, ИндексСтрокиБ, ИндексСтрокиА);
				Результат.ИмяОбласти	= НайденаОбласть.Имя;

			КонецЕсли;
			
			ИндексСтрокиБ = ИндексСтрокиБ + 1;
		КонецЦикла;
...
#Область ЗаполнениеОбластиТабличногоДокументе_
		...
		_ПостфиксНомераОбласти 	= СтрЗаменить(ОписаниеДобавления.Имя, ОписаниеДобавления.Префикс, "");
		_ИмяДобавленнойОбласти 	= СтрШаблон("П%1%2", _НомерОбласти, _ПостфиксНомераОбласти);
		...
		_текЗначениеСУчетомТипа = Источник[_ИмяКолонки];
		Если Тип(_ТабДокОбласти.Области[_ИмяДобавленнойОбласти].ТипЗначения) = Тип(Новый ОписаниеТипов("Дата")) Тогда			
			_ВалиднаяСтрДаты		= СтрЗаменить(Источник[_ИмяКолонки], ",", ".");
			_ВалиднаяСтрДаты		= СтрЗаменить(_ВалиднаяСтрДаты, "/", ".");
			_ВалиднаяСтрДаты		= СтрЗаменить(_ВалиднаяСтрДаты, "\", ".");
			_ВалиднаяСтрДаты		= СтрЗаменить(_ВалиднаяСтрДаты, " ", "");
			_мЧастиДаты				= СтрРазделить( СокрЛП( _ВалиднаяСтрДаты ), ".");
			_ПредставлениеГод		= Формат(Число(_мЧастиДаты[2]), "ЧЦ=4; ЧГ=0");
			_ПредставлениеМесяц		= Формат(Число(_мЧастиДаты[1]), "ЧЦ=2; ЧВН=; ЧГ=0");
			_ПредставлениеДень		= Формат(Число(_мЧастиДаты[0]), "ЧЦ=2; ЧВН=; ЧГ=0");
			_СтрДатаВРеверсе		= ?( СтрДлина( _мЧастиДаты[2]) = 4, СтрШаблон("%1%2%3", _ПредставлениеГод, _ПредставлениеМесяц, _ПредставлениеДень), Источник[_ИмяКолонки]);
			_текЗначениеСУчетомТипа = Дата(_СтрДатаВРеверсе);
		ИначеЕсли Тип(_ТабДокОбласти.Области[_ИмяДобавленнойОбласти].ТипЗначения) = Тип(Новый ОписаниеТипов("Число")) Тогда
			_текЗначениеСУчетомТипа = ?( ЗначениеЗаполнено( СтрЗаменить(Источник[_ИмяКолонки], " ", "") ), Число(Источник[_ИмяКолонки]), 0);
		КонецЕсли;
		
		_ТабДокОбласти.Области[_ИмяДобавленнойОбласти].Значение = _текЗначениеСУчетомТипа;
#КонецОбласти

#Область РасчетОбластьДляФормированияИтогов

		_ИмяРазделаРасчета  = "Реестр";
		_ИмяОбластиРасчета	= СтрРазделить(_ИмяДобавленнойОбласти, "_")[0]; // "П1000008";
		Расчет(ЭтаФорма, _ИмяРазделаРасчета, _ИмяОбластиРасчета);
