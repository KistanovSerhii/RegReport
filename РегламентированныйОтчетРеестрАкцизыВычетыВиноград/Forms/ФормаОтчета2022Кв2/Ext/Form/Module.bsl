...
Процедура ВставитьДополнительнуюСтроку(Знач Копирование = Ложь)
	...
	Если ИмяТекОбласти = "R1C1" Тогда
		ИмяТекОбласти = "ДобавитьСтрокуП10000_1";
	КонецЕсли;
...
Функция СтрокаСКоординатамиПоказателя(Знач Координаты)
	...
	ИначеЕсли ТипЗнч(Координаты) = Тип("Строка") Тогда
		...
		Если МассивСтрок.Количество() >= КолСтрок - 1 Тогда
...
#Область ЗаполнениеОтчетаИзВнешнихДанных

&НаКлиенте
Процедура ЗаполнитьВнешнимиДанными(Команда)
	
	ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестр();
	
КонецПроцедуры

#Область ЗаполнениеОтчетаРазделаРеестр

&НаКлиенте
Процедура ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестр()
	
	// Необходимо выбрать страницу для загрузки (например: раздел "Реестр -> Стр. 1")
	ИмяРазделаДляЗагрузкиДанных = "Стр. 1";
	Если ЭтаФорма.ЭтаФорма.ТекущийЭлемент.ТекущиеДанные.КолонкаРазделыОтчета <> ИмяРазделаДляЗагрузкиДанных Тогда
		Сообщение = Новый СообщениеПользователю;
	...
	ИТ_ТД.Прочитать(ДиалогВыбора("чтение"));

	ОжидаемыеИменаКолонок 		= ОписаниеДанныхОтчета();
	НомерСтрокиЗаголовкаКолонок	= ПолучитьНачалоЗаголовкаКолонокВнешнихДанных(ОжидаемыеИменаКолонок);
	
	Если НомерСтрокиЗаголовкаКолонок = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
	...
	НачалоВнешнихДанных = НомерСтрокиЗаголовкаКолонок + 1;
	НаборДанных			= ПолучитьДанныеИзТабДокумента(НачалоВнешнихДанных, НомерСтрокиЗаголовкаКолонок);
	
	// ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестрНаСервере(НаборДанных);
	ДобавитьДанныеВРазделОтчетаРеестр(НаборДанных);
	
	// Тестирование наполнения
	//тНаборДанных = ПолучитьТестовыйНаборДанных();
	//ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестрНаСервере(тНаборДанных);
	
КонецПроцедуры

&НаСервере
Функция ОписаниеУровнейВСтрокеМакетаРеестр()
...
Процедура ДобавитьСтрокуВРазделОтчетаРеестр(мДанных, ИндексДанных, УровеньВСтроке)
	...
	ОписаниеУровнейВСтроке	= ОписаниеУровнейВСтрокеМакетаРеестр();
...
&НаСервере
Процедура ДобавитьДанныеВРазделОтчетаРеестр(мДанных)
	...
	Пока ИтераторДанных < мДанных.Количество() Цикл
		...
		ДобавитьСтрокуВРазделОтчетаРеестр(мДанных, ИтераторДанных, УровеньВСтроке);
...
#Область ПодготовкаВнешнихДанных

// Функция - Получить данные из таб документа
//
// Параметры:
//  НачалоВнешнихДанных			 - Число - Индекс строки с которой начинаются данные
//  ИндексСтрокиЗаголовокКолонок - Число - Индекс строки заголовка (нумерация колонок)
// 
// Возвращаемое значение:
//  Массив Из Соответствие (см. ОписаниеДанныхОтчета)
//
&НаКлиенте
Функция ПолучитьДанныеИзТабДокумента(НачалоВнешнихДанных, ИндексСтрокиЗаголовокКолонок)
	
	мДанных = Новый Массив;
		
	Для НомерСтроки = НачалоВнешнихДанных По ИТ_ТД.ВысотаТаблицы Цикл
		
		ОписаниеДанных 	= ОписаниеДанныхОтчета();
		
		Для НомерКолонки = 1 по ИТ_ТД.ШиринаТаблицы Цикл
			
			ИмяЗаголовка 	= СОКРЛП( ИТ_ТД.Область(ИндексСтрокиЗаголовокКолонок, НомерКолонки).Текст );
			текЗнч			= ИТ_ТД.Область(НомерСтроки, НомерКолонки).Текст;
			ИсключитьСимвол = "Х";
			Если ОписаниеДанных.Получить(ИмяЗаголовка) <> Неопределено
				И ЗначениеЗаполнено( СОКРЛП(текЗнч) ) 
				И текЗнч <> ИсключитьСимвол Тогда
					ОписаниеДанных[ИмяЗаголовка] = текЗнч;
			КонецЕсли;

		КонецЦикла;
		
		Если СоответствиеЗаполнено(ОписаниеДанных) Тогда
			
			// Необходимо сгруппировать данные в соответствии с заданием
			// Данные = ГруппироватьДанные(ОписаниеДанных);
			// мДанных.Добавить(Данные);
			
			мДанных.Добавить(ОписаниеДанных);
		КонецЕсли;
...
&НаКлиенте
Функция СоответствиеЗаполнено(С)
		
	Заполнено 		= Ложь;	
	КолЗаполненых 	= 0;
	
	Для Каждого э Из С Цикл
...
// Функция - Получить начало данных в таб док
//
// Параметры:
//  ВалидныйЗаголовокДанных - Соответствие Где ключ это ожидаемое имя колонки
// 
// Возвращаемое значение:
//  Число - Индекс первой строки с данными
//
&НаКлиенте
Функция ПолучитьНачалоЗаголовкаКолонокВнешнихДанных(ВалидныйЗаголовокДанных)
			
	НомерСтрокиСДанными 			= Неопределено;
	КоличествоНумерованыхКолонок 	= 0;
	
	Для НомерСтроки = 1 по ИТ_ТД.ВысотаТаблицы Цикл
		
		КолВалидных = 0;
		Для НомерКолонки = 1 по ИТ_ТД.ШиринаТаблицы Цикл
			
			ИмяЗаголовка = СОКРЛП( ИТ_ТД.Область(НомерСтроки, НомерКолонки).Текст );			
			
			Если ВалидныйЗаголовокДанных.Получить(ИмяЗаголовка) <> Неопределено Тогда
				КолВалидных = КолВалидных + 1;
			КонецЕсли;

			Если КолВалидных = ВалидныйЗаголовокДанных.Количество() Тогда
				Возврат НомерСтроки;
			КонецЕсли;
...
// "ПолучитьНачалоЗаголовкаКолонокВнешнихДанных" проверяется наличие ключа!
// 
// Возвращаемое значение:
//  Соответствие ИЗ Ключ - номер колонки отчета, Значение - знч колонки
//
&НаКлиенте
Функция ОписаниеДанныхОтчета()
...
// Для проверки авто. наполнения
&НаКлиенте
Функция ПолучитьТестовыйНаборДанных()
	
	мДанных 		= Новый Массив;	
	ВнешниеДанные 	= ОписаниеДанныхОтчета();
	
    // Таблица отчета начинается с четвертой колонки!
	ВнешниеДанные["4"] = "Test4";
	ВнешниеДанные["5"] = "Test";
	ВнешниеДанные["6"] = "Test";
	ВнешниеДанные["7"] = "Test";
	ВнешниеДанные["8"] = "Test";
	ВнешниеДанные["9"] = "Test";
	ВнешниеДанные["10"] = "Test";
	ВнешниеДанные["11"] = "Test11";
	
	ВнешниеДанные["12"] = "Test";
	ВнешниеДанные["13"] = "Test";
	ВнешниеДанные["14"] = "Test14";
	
	ВнешниеДанные["15"] = "Test15";
	ВнешниеДанные["16"] = "Test";
	ВнешниеДанные["17"] = "Test";
	ВнешниеДанные["18"] = "Test";
	ВнешниеДанные["19"] = "Test";
	ВнешниеДанные["20"] = "Test";
	ВнешниеДанные["21"] = "Test";
	ВнешниеДанные["22"] = "Test";
	ВнешниеДанные["23"] = "Test";
	ВнешниеДанные["24"] = "Test";
	ВнешниеДанные["25"] = "Test";
	ВнешниеДанные["26"] = "Test26";

	// Строки
    мДанных.Добавить(ВнешниеДанные);
	мДанных.Добавить(ВнешниеДанные);
	мДанных.Добавить(ВнешниеДанные);
