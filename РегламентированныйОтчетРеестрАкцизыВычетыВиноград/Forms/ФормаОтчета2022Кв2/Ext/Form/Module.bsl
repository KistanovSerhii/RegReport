...
// Процедура - Заполнить отчет внешними данными на сервере
//
// Параметры:
...
//
&НаСервере
Процедура ЗаполнитьОтчетВнешнимиДаннымиНаСервере(мДанных)
	...
	Пока ИтераторДанных < мДанных.Количество() Цикл		
		...
		ПрефиксЧислоЗнаков	= 6;
		ИтераторКолонок 	= 4;
		
		Пока ИтераторКолонок < мДанных[ИтераторДанных].Количество() Цикл
			
			УровеньСтроки = ПостфиксУровень0;
			ПрефиксУровня = ПрефиксУровня0;
			
			// ----------------------------------------------------			
			Если ИтераторКолонок > 11 И ИтераторКолонок < 15 Тогда
				УровеньСтроки	= ПостфиксУровень1;
				ПрефиксУровня 	= ПрефиксУровня1;
				ПрефиксЧислоЗнаков	= 5;
			КонецЕсли;
			
			Если ИтераторКолонок > 14 Тогда
				УровеньСтроки 	= ПостфиксУровень2;
				ПрефиксУровня	= ПрефиксУровня2;
				ПрефиксЧислоЗнаков	= 5;
			КонецЕсли;
			// ----------------------------------------------------
			
			ИмяТекущейКолонки = Формат(ИтераторКолонок, "ЧЦ=" + ПрефиксЧислоЗнаков + "; ЧВН=; ЧГ=0; ЧФ=" + ПрефиксУровня + "Ч_")
			+ НомерСтрокиОтчета
			+ УровеньСтроки;
			
			ТДОбласти[ИмяТекущейКолонки].Значение = мДанных[ИтераторДанных][ Строка(ИтераторКолонок) ];
...
Процедура ЗаполнитьВнешнимиДанными(Команда)
	...
	ЗаполнитьОтчетВнешнимиДаннымиНаСервере(НаборДанных);
	
	// Тестирование наполнения
	//тНаборДанных = ПолучитьТестовыйНаборДанных();
	//ЗаполнитьОтчетВнешнимиДаннымиНаСервере(тНаборДанных);
