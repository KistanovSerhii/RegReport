#Область КодТиповойКонфигурации
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.реестр акц.виноград") Тогда
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС, КПП, Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт
	...
	СтруктураРеквизитовФормы.мСохраненныйДок.ИсточникОтчета
		= Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""),
		СтрНайти(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].Имя;
	СтруктураРеквизитовФормы.мСохраненныйДок.НаименованиеОтчета
		= Метаданные.Отчеты[Сред(Лев(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""),
		СтрНайти(СтрЗаменить(ЭтаФорма.ИмяФормы, "Внешний", ""), ".Форма.") - 1), 7)].ОсновнаяФорма.Синоним;
...
Процедура Выгрузить(Команда)
	...
	|от 01.03.2022 № ЕД-7-15/173@, будет
...
// ITT # 242 16.07.2022
#Область ITT_242_ЗаполнениеОтчетаИзВнешнихДанных

#Область КомандыТочкаВхода

&НаКлиенте
Процедура ЗаполнитьВнешнимиДанными(Команда)
	
	_ПравилаГруппировки	= ПравилаГруппировкиРазделаОтчета_Реестр();

#Область ТестированиеИТТ242

	// ---------------- ТЕСТ ------------------ >
	ЭтоТест = Ложь;
	
	Если ЭтоТест Тогда			
		ДобавитьТЕСТОВЫЕДанныеВРазделеОтчета_Реестр_НеСервере(_ПравилаГруппировки);		
		Возврат;
	КонецЕсли;
	// < -------------- ТЕСТ -------------------		
...
#Область ЗагрузкаИзТабличногоДокумента
	...
	ТабличныйДокументДляЗагрузкиДанных = ИТ_ТД;
	
	Если Не ПрочитатьДанныеИзФайлаЭкселВТабДок(ТабличныйДокументДляЗагрузкиДанных) Тогда
		Возврат;
	КонецЕсли;
	
	ОжидаемыеИменаЗаголовокаДанных	= ОписаниеЗаголовкаДанныхВРазделеОтчета_Реестр();
	НомерСтрокиЗаголовкаДанных		= ПолучитьИзТабДокНомерСтрокиСодержащегоЗаголовокДанных(ТабличныйДокументДляЗагрузкиДанных, ОжидаемыеИменаЗаголовокаДанных);
	
	Если НомерСтрокиЗаголовкаДанных = Неопределено Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = ""Ошибка: Убедитесь что заголовки данных в файле excel совподают с заколовками рег. отчета (раздел Реестр)!"" ");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	
	_ОписаниеЗаголовкаДанных = Новый Структура;
	_ОписаниеЗаголовкаДанных.Вставить("Индекс");
	_ОписаниеЗаголовкаДанных.Вставить("НачалоДанных");
	_ОписаниеЗаголовкаДанных.Вставить("ПрефиксКолонок");
	_ОписаниеЗаголовкаДанных.Вставить("Имя");
	
	
	_ОписаниеЗаголовкаДанных.Индекс 		= НомерСтрокиЗаголовкаДанных;
	_ОписаниеЗаголовкаДанных.НачалоДанных 	= НомерСтрокиЗаголовкаДанных + 1;
	_ОписаниеЗаголовкаДанных.ПрефиксКолонок	= Получить_ПрефиксКолонокТЗ();
	_ОписаниеЗаголовкаДанных.Имя 			= ОжидаемыеИменаЗаголовокаДанных;
	
	ДобавитьДанныеТабДокВРазделеОтчета_Реестр_НеСервере(ТабличныйДокументДляЗагрузкиДанных, _ОписаниеЗаголовкаДанных);
...
#Область ПодготовкаИЧтениеДанныхИзТабДок

// Функция - ПрочитатьДанныеИзФайлаЭкселВТабДок
//
// Параметры:
//  ТабДок - Табличный документ - ТабДок в который будут прочитаны данные из файла эксел
// 
// Возвращаемое значение:
//  Булево - Истина если данные удачно прочитаны
//
&НаКлиенте
Функция ПрочитатьДанныеИзФайлаЭкселВТабДок(ТабДок)
	
	// Необходимо выбрать страницу для загрузки (например: раздел "Реестр -> Стр. 1")
	ИмяРазделаДляЗагрузкиДанных = "Реестр";
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела <> ИмяРазделаДляЗагрузкиДанных Тогда
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = ""Необходимо выбрать страницу для загрузки (например: раздел Реестр -> Стр. 1)!"" ");
		Сообщение.Сообщить();
	...
	Попытка
		ТабДок.Прочитать(ДиалогВыбора("чтение"));
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = ""Ошибка чтения файла! Убедитесь что у вас запущен 'ТОЛСТЫЙ КЛИЕНТ' И открываемяй ФАЙЛ ЗАКРЫТ!"" ");
		Сообщение.Сообщить();
...
// Функция - Описание данных отчета
// ВАЖНО инициализировать пустой строкой так как в методе -
// "ПолучитьИзТабДокНомерСтрокиСодержащегоЗаголовокДанных" проверяется наличие ключа!
// 
// Возвращаемое значение:
//  Соответствие ИЗ Ключ - номер колонки отчета, Значение - знч колонки
//
&НаКлиенте
Функция ОписаниеЗаголовкаДанныхВРазделеОтчета_Реестр()

	ВнешниеДанные = Новый Соответствие;
	
	// Таблица отчета начинается с четвертой колонки!
	ВнешниеДанные.Вставить("4",  "");
	ВнешниеДанные.Вставить("5",  "");
	ВнешниеДанные.Вставить("6",  "");
	ВнешниеДанные.Вставить("7",  "");
	ВнешниеДанные.Вставить("8",  "");
	ВнешниеДанные.Вставить("9",  "");
	ВнешниеДанные.Вставить("10", "");
	ВнешниеДанные.Вставить("11", "");
	
	ВнешниеДанные.Вставить("12", "");
	ВнешниеДанные.Вставить("13", "");
	ВнешниеДанные.Вставить("14", "");
	
	ВнешниеДанные.Вставить("15", "");
	ВнешниеДанные.Вставить("16", "");
	ВнешниеДанные.Вставить("17", "");
	ВнешниеДанные.Вставить("18", "");
	ВнешниеДанные.Вставить("19", "");
	ВнешниеДанные.Вставить("20", "");
	ВнешниеДанные.Вставить("21", "");
	ВнешниеДанные.Вставить("22", "");
	ВнешниеДанные.Вставить("23", "");
	ВнешниеДанные.Вставить("24", "");
	ВнешниеДанные.Вставить("25", "");
	ВнешниеДанные.Вставить("26", "");
	
	Возврат ВнешниеДанные;
	
КонецФункции

// Функция - ПолучитьИзТабДокНомерСтрокиСодержащегоЗаголовокДанных
//
// Параметры:
//	ТабДок - Табличный документ в котором необходимо найти заголовок данных
//  ОжидаемыеИменаЗаголовокаДанных - Соответствие Где ключ это ожидаемое имя колонки заголовка
// 
// Возвращаемое значение:
//  Число - Индекс строки заголовка
//
&НаКлиенте
Функция ПолучитьИзТабДокНомерСтрокиСодержащегоЗаголовокДанных(ТабДок, ОжидаемыеИменаЗаголовокаДанных)
	
	НомерСтрокиЗаголовкаДанных		= Неопределено;
	КоличествоНумерованыхКолонок 	= 0;
	...
	Для НомерСтроки = 1 по ТабДок.ВысотаТаблицы Цикл
		...
		Для НомерКолонки = 1 по ТабДок.ШиринаТаблицы Цикл
			...
			ИмяЗаголовка = СОКРЛП( ТабДок.Область(НомерСтроки, НомерКолонки).Текст );			
			
			Если ОжидаемыеИменаЗаголовокаДанных.Получить(ИмяЗаголовка) <> Неопределено Тогда
				КолВалидных = КолВалидных + 1;
			КонецЕсли;

			Если КолВалидных = ОжидаемыеИменаЗаголовокаДанных.Количество() Тогда
				Возврат НомерСтроки;
			КонецЕсли;
	...
	Если НомерСтрокиЗаголовкаДанных = Неопределено Тогда
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = ""Внешний файл должен содержать нумерацию колонок как заголовок данных!""");
		Сообщение.Сообщить();
	...
	Возврат НомерСтрокиЗаголовкаДанных;
	
КонецФункции

#Область ДиалогВыбораФайла
		
&AtClient
Функция ДиалогВыбора(режим)
	
	ДиалогВыбора 			= Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
	ДиалогВыбора.Заголовок 	= ? (Режим="чтение", "Прочитать","Сохранить") ;
	ДиалогВыбора.Фильтр 	= "Документ Microsoft Excell (*.xls;*.xlsx)|*.xls;*.xlsx";
	
	Если ДиалогВыбора.Choose() Тогда
...
#Область ПреобразованиеТабДокВТабЗнач

// Процедура - ПреобразоватьТабДокВТаблицуЗначений
//
// Параметры:
//  ТабДок - Табличный документ - таб. док. данные которого должны быть загружены в ТЗ
//  _Заголовок - Структура ИЗ
//  *Индекс - Число - 
//  *НачалоДанных - Число - 
//	*ПрефиксКолонок - Строка - см. Получить_ПрефиксКолонокТЗ
//  *Имя - Соответствие - Имя колонок 
// 
// Возвращаемое значение:
//  Таблица значений
//
&НаСервере
Функция ПреобразоватьТабДокВТаблицуЗначений(ТабДок, _Заголовок, _ПравилаГруппировки = Неопределено)
	
	// _ГруппировкаКолонок	= _ПравилаГруппировки.Получить(_УровнейГруппировки);
	
	Если _Заголовок.НачалоДанных = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// Создать Таблицу значений с наименованием колонок идентичным (ожидаемым) табличному документу (в который прочитан файл excel) >
	_ТабЗначений 	= Новый ТаблицаЗначений;
	_ЗаголовкиТЗ 	= Новый Структура;	
	
	Для НомерКолонки = 1 по ТабДок.ШиринаТаблицы Цикл
		
		_ТекстЗаголовок			= ТабДок.Область(_Заголовок.Индекс, НомерКолонки).Текст;
		_ЗаголовокСуществует	= _Заголовок.Имя.Получить(_ТекстЗаголовок);
		
		Если _ЗаголовокСуществует = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		_ИмяТекКолонки = _Заголовок.ПрефиксКолонок + СтрЗаменить( СокрЛП( Строка(_ТекстЗаголовок) ), " ", "");
		
		_ТабЗначений.Колонки.Добавить(_ИмяТекКолонки, Новый ОписаниеТипов("Строка"), _ТекстЗаголовок);
		_ЗаголовкиТЗ.Вставить(_ИмяТекКолонки);
	КонецЦикла;
	// Создать Таблицу значений с наименованием колонок идентичным (ожидаемым) табличному документу (в который прочитан файл excel) <
	
	// Добавть Колонки которые позволят выполнить заполнение по РАЗЛИЧНЫЕ и дабавить данные ступенчато (в соответствии с макетом рег. отчета) >
	Если _ПравилаГруппировки <> Неопределено Тогда
		
		_ИмяПрефиксУровня = Получить_ПрефиксУровняГруппировкиТЗ();
		
		Для Каждого Уровня Из _ПравилаГруппировки Цикл
			_ИмяУровня = СтрШаблон("%1%2", _ИмяПрефиксУровня, Уровня.Ключ);
			_ТабЗначений.Колонки.Добавить(_ИмяУровня, Новый ОписаниеТипов("Строка"), Строка(Уровня.Ключ));
			_ЗаголовкиТЗ.Вставить(_ИмяУровня);
		КонецЦикла;
	КонецЕсли;
	// <

	// По какой то причине читается пустая строка в концу таб. документа с знч "Х",
	// по этой причине добавляем данную переменную, что позволит пропустить пустую строку
	ИсключитьСтрокиСодержащиеТолькоСимвол = "Х";

	#Область ОбходТабличногоДокументаСЗаполнениемТЗ
	
	// Преобразование табличного документа в таблицу значений >
	Для НомерСтроки = _Заголовок.НачалоДанных По ТабДок.ВысотаТаблицы Цикл
		
		// 1. Структура для заполнения тек. строки тз
		_ДанныеТекСтрокиТЗ = Новый Структура;
		Для каждого КолонкаЗаголовок Из _ЗаголовкиТЗ Цикл
			_ДанныеТекСтрокиТЗ.Вставить(КолонкаЗаголовок.Ключ, "");
		КонецЦикла;
		
		Для НомерКолонки = 1 по ТабДок.ШиринаТаблицы Цикл
			
			_ИмяКолонки 				= ТабДок.Область(_Заголовок.Индекс, НомерКолонки).Текст;
			_текЗнч						= ТабДок.Область(НомерСтроки, НомерКолонки).Текст;
			текЗнчПроверкаЗаполнения 	= СтрЗаменить( СОКРЛП(_текЗнч), " ", "");			
			ЗнчТекущегоУровня		= _Заголовок.Имя.Получить(_ИмяКолонки);
			
			Если ЗнчТекущегоУровня <> Неопределено
				И ЗначениеЗаполнено( текЗнчПроверкаЗаполнения ) И ВРег(текЗнчПроверкаЗаполнения) <> ИсключитьСтрокиСодержащиеТолькоСимвол Тогда
				
				// 1.2 Заполняем данные (структура) для тек. строки тз
				_ДанныеТекСтрокиТЗ[_Заголовок.ПрефиксКолонок + Строка(_ИмяКолонки)] = _текЗнч;				
			КонецЕсли;
		КонецЦикла;
		
		Если КоллекцияЗначенийЗаполнена(_ДанныеТекСтрокиТЗ) Тогда
			
			// 2. Наполнение колонки "Уровень" которое необходимо для заполнения РАЗЛИЧНЫМИ >
			Если _ПравилаГруппировки <> Неопределено Тогда
				
				ИтераторПравилГруппировки = 0;
				Пока ИтераторПравилГруппировки < _ПравилаГруппировки.Количество() Цикл					
					
					_ЗнчУровня = _ПравилаГруппировки[ИтераторПравилГруппировки];
					
					Для Каждого _СтрТЗ Из _ДанныеТекСтрокиТЗ Цикл
						
						Если НЕ СтрНайти(_СтрТЗ.Ключ, _Заголовок.ПрефиксКолонок) Тогда
							Продолжить;
						КонецЕсли;
						
						_текКлюч = Число(СтрЗаменить(_СтрТЗ.Ключ, _Заголовок.ПрефиксКолонок, ""));
						Если  _текКлюч >= _ЗнчУровня.С И _текКлюч <= _ЗнчУровня.ПО Тогда
							_ИмяТекУровня 	= СтрШаблон("%1%2", _ИмяПрефиксУровня, _ЗнчУровня.Уровень);
							_знчСтруктуры	= _ДанныеТекСтрокиТЗ[_СтрТЗ.Ключ];
							_ДанныеТекСтрокиТЗ[_ИмяТекУровня] = _ДанныеТекСтрокиТЗ[_ИмяТекУровня] + ВРег( СтрЗаменить( СокрЛП(_знчСтруктуры), " ", "") );
						КонецЕсли;

					КонецЦикла;
					
					ИтераторПравилГруппировки = ИтераторПравилГруппировки + 1;
				КонецЦикла;
			
			КонецЕсли;
			
			новСтрокаТЗ = _ТабЗначений.Добавить();
			// 1.3 Заполняем тз данными из тек. строки (структура)
			ЗаполнитьЗначенияСвойств(новСтрокаТЗ, _ДанныеТекСтрокиТЗ);
		КонецЕсли;
		
	КонецЦикла;
	// Преобразование табличного документа в таблицу значений <
	
	Возврат _ТабЗначений;
	
	#КонецОбласти
	
КонецФункции

// Так как имена колонок ТЗ не могут быть числом нам необходимо добавить к ним какой либо текст
&НаСервере
Функция Получить_ПрефиксКолонокТЗ()
	Возврат "TableColumn";
КонецФункции

// Так как имена колонок ТЗ не могут быть числом нам необходимо добавить к ним какой либо текст
&НаСервере
Функция Получить_ПрефиксУровняГруппировкиТЗ()
	Возврат "GroupLevel";
КонецФункции

// Функция - КоллекцияЗначенийЗаполнена
//
// Параметры:
//  Данные - Соответствие - Коллекция значений
// 
// Возвращаемое значение:
//  Булево - Истина если хотябы один ключ имеет заполненое знч
//
&НаСервере
Функция КоллекцияЗначенийЗаполнена(Данные)
		
	Заполнено 		= Ложь;	
	КолЗаполненых 	= 0;
	
	Для Каждого э Из Данные Цикл
		Если ЗначениеЗаполнено(э.Значение) Тогда
			КолЗаполненых = КолЗаполненых + 1;
		КонецЕсли;
	КонецЦикла;
	
	Возврат КолЗаполненых > 1;
	
КонецФункции

// Описывает с какой по какую колонку необходимо группировать
&НаСервере
Функция ПравилаГруппировкиРазделаОтчета_Реестр()
	
	КолонкиУровень0 = Новый Структура;
	КолонкиУровень1 = Новый Структура;
	КолонкиУровень2 = Новый Структура;
	
	КолонкиУровень0.Вставить("С");
	КолонкиУровень0.Вставить("ПО");
	КолонкиУровень0.Вставить("Уровень");
	
	КолонкиУровень1.Вставить("С");
	КолонкиУровень1.Вставить("ПО");
	КолонкиУровень1.Вставить("Уровень");
	
	КолонкиУровень2.Вставить("С");
	КолонкиУровень2.Вставить("ПО");
	КолонкиУровень2.Вставить("Уровень");
	
	КолонкиУровень0.С		= 4;
	КолонкиУровень0.ПО		= 11;
	КолонкиУровень0.Уровень	= 0;
	
	КолонкиУровень1.С		= 12;
	КолонкиУровень1.ПО		= 14;
	КолонкиУровень1.Уровень	= 1;
	
	КолонкиУровень2.С		= 15;
	КолонкиУровень2.ПО		= 26;
	КолонкиУровень2.Уровень	= 2;
	
	ОписаниеСтупеней = Новый Соответствие;
	ОписаниеСтупеней.Вставить(0, КолонкиУровень0);
	ОписаниеСтупеней.Вставить(1, КолонкиУровень1);
	ОписаниеСтупеней.Вставить(2, КолонкиУровень2);
	
	Возврат ОписаниеСтупеней;

КонецФункции

#КонецОбласти

#Область ДобавитьДанныеВРазделОтчета_Реестр

// Процедура - ДобавитьДанныеТабДокВРазделеОтчета_Реестр_НеСервере
//
// Параметры:
//  ТабДок - Табличный документ - таб. док. данные которого должны быть загружены в ТЗ
//  _Заголовок - Структура ИЗ
...
//
&НаСервере
Процедура ДобавитьДанныеТабДокВРазделеОтчета_Реестр_НеСервере(ТабДок, _Заголовок)
	
	_ПравилаГруппировки	= ПравилаГруппировкиРазделаОтчета_Реестр();	
	_ТабЗначений 		= ПреобразоватьТабДокВТаблицуЗначений(ТабДок, _Заголовок, _ПравилаГруппировки);
	
	Если _ТабЗначений = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере(_ТабЗначений, _ПравилаГруппировки);
	
КонецПроцедуры

// Процедура - ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере
//
// Параметры:
//  мДанных	 - Массив ИЗ Соответствие
//	*Индекс массива - Это строка отчета
//	*Ключ соответствия - Это номер колонки отчета
//
//	Комментарий: см процедуру "ОписаниеЗаголовкаДанныхВРазделеОтчета_Реестр"
//
&НаСервере
Процедура ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере(ИсточникДанных, _ПравилаГруппировки)
	
	Если ТипЗнч(ИсточникДанных) <> Тип("ТаблицаЗначений") Тогда
		Возврат;
	КонецЕсли;
	
	_ТабличныйДокументРегОтчета = ЭтаФорма.ТабличныйДокумент;
	_ОбработанныеСтроки			= Новый Массив;
	_ПрефиксУровняГруппировкиТЗ = Получить_ПрефиксУровняГруппировкиТЗ();
	_ИтераторСтрок				= 0;

	// Перебор СТРОК (движение по строчно вниз)
	Для Каждого _СтрокаТЗ Из ИсточникДанных Цикл
		
		_ИмяГруппировки0 	= СтрШаблон("%1%2", _ПрефиксУровняГруппировкиТЗ, 0);			
		_ОтборУровней 		= Новый Структура();			
		_ОтборУровней.Вставить(_ИмяГруппировки0, _СтрокаТЗ[_ИмяГруппировки0]);
		
		// ТАК КАК С КАЖДОЙ СТРОКОЙ МЫ ДЕЛАЕМ ОТБОР ПО УРОВНЮ НОЛЬ ТО НА ДОСТАТОЧНО ОДНОГО РАЗА
		Если _ОбработанныеСтроки.Найти(_СтрокаТЗ[_ИмяГруппировки0]) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		// ОТБОР СТРОК УРОВНЯ 0 (все записи содержащие знч уровня 0)
		_мВсеЗаписиТЗПоЗначениюУровеня0 = ИсточникДанных.НайтиСтроки(_ОтборУровней);		
		
		
		_УжеДобавлен 						= Новый Массив;
		ИтераторЗаписейТЗПоЗначениюУровеня0 = 0;
		ОсновнаяСтрокаДобавлена				= Ложь;
		
		// Перебор УРОВНЕЙ (движение по срокам в НИЗ)
		Пока ИтераторЗаписейТЗПоЗначениюУровеня0 < _мВсеЗаписиТЗПоЗначениюУровеня0.Количество() Цикл

			ИтераторУровняN = 0;
			
			// Перебор УРОВНЕЙ (движение по колонкам в БОК)
			Пока ИтераторУровняN < _ПравилаГруппировки.Количество() Цикл
				
				_ИмяТекГруппировки 	= СтрШаблон("%1%2", _ПрефиксУровняГруппировкиТЗ, ИтераторУровняN);
				
				Если _УжеДобавлен.Найти(_мВсеЗаписиТЗПоЗначениюУровеня0[ИтераторЗаписейТЗПоЗначениюУровеня0][_ИмяТекГруппировки]) <> Неопределено Тогда
					ИтераторУровняN = ИтераторУровняN + 1;
					Продолжить;
				КонецЕсли;								
				
				// ПЕРВАЯ ОСНОВНАЯ ЧАСТЬ ОБРАБОТКИ (ДОБАВЛЕНИЕ ОБЛАСТЕЙ В МАКЕТ РЕГ. ОТЧЕТА) --------------->
				ОписаниеДобавления = _ДобавитьСтрокуВТабличныйДокумент(_ТабличныйДокументРегОтчета, ИтераторУровняN, ИтераторЗаписейТЗПоЗначениюУровеня0, _ИтераторСтрок);
				// <---------------
				
				// ВТОРАЯ ОСНОВНАЯ ЧАСТЬ ОБРАБОТКИ (ЗАПОЛНЕНИЕ ОБЛАСТЕЙ В МАКЕТ РЕГ. ОТЧЕТА) --------------->
				ЗаполнитьСтрокуТабличногоДокумента(_ТабличныйДокументРегОтчета, _мВсеЗаписиТЗПоЗначениюУровеня0[ИтераторЗаписейТЗПоЗначениюУровеня0], ОписаниеДобавления, _ПравилаГруппировки);
				// <---------------				
				
				_УжеДобавлен.Добавить(_мВсеЗаписиТЗПоЗначениюУровеня0[ИтераторЗаписейТЗПоЗначениюУровеня0][_ИмяТекГруппировки]);
				
				ИтераторУровняN = ИтераторУровняN + 1;	
			КонецЦикла;
			
			ИтераторЗаписейТЗПоЗначениюУровеня0 = ИтераторЗаписейТЗПоЗначениюУровеня0 + 1;
		КонецЦикла;
		
		_ОбработанныеСтроки.Добавить(_СтрокаТЗ[_ИмяГруппировки0]);
		
		_ИтераторСтрок = _ИтераторСтрок + 1;
...
#Область ОСНОВНАЯ_ЧАСТЬ_ОБРАБОТКИ_ДОБАВЛЕНИЕОбластейЗАПОЛНЕНИЕОбластей

&НаСервере
Функция _ДобавитьСтрокуВТабличныйДокумент(ТабДок, Уровень, ИтераторЗаписейТЗПоЗначениюУровеня0, ИтераторСамогоВерхнегоСлоя)
	
	Результат		= Новый Структура;
	
	Результат.Вставить("Имя");
	Результат.Вставить("Префикс");
	Результат.Вставить("ИндексНового");
	Результат.Вставить("НаходитсяВСтроке");
	Результат.Вставить("Номер");
	Результат.Вставить("Уровень");
	
	ТабДокОбласти 	= ТабДок.Области;
	
	ОписаниеУровняN	= Новый Структура;
	ОписаниеУровняN.Вставить("Префикс");
	ОписаниеУровняN.Вставить("НаходитсяВСтроке");
	
	СсылкаДобавить	= "ДобавитьСтрокуЗначок";
	КнопкаДобавить	= "ДобавитьСтроку";
	
	ПрефиксУровня0	= "П10000";
	ПрефиксУровня1	= "П11000";
	ПрефиксУровня2	= "П12000";
	
	Уровня0Индекс	= Неопределено;
	Уровня1Индекс	= Неопределено;
	Уровня2Индекс	= Неопределено;
	
	Уровня0Индекс = НайтиКрайнююОбластьДанных(ТабДокОбласти, СсылкаДобавить, ПрефиксУровня0);
	Если Уровня0Индекс = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ОписаниеУровняN.Префикс				= ПрефиксУровня1;
	ОписаниеУровняN.НаходитсяВСтроке	= Уровня0Индекс;	
	Уровня1Индекс = НайтиКрайнююОбластьДанных(ТабДокОбласти, СсылкаДобавить, Уровня0Индекс, ОписаниеУровняN);
	
	ОписаниеУровняN.Префикс				= ПрефиксУровня2;
	ОписаниеУровняN.НаходитсяВСтроке	= Уровня0Индекс;
	Уровня2Индекс = НайтиКрайнююОбластьДанных(ТабДокОбласти, СсылкаДобавить, Уровня0Индекс, ОписаниеУровняN);	
		
	ИмяТекОбласти	= Неопределено;
	
	Если Уровень = 0 Тогда
		ИмяКнопкиТекОбласти 	= СтрШаблон("%1%2_%3", 	  СсылкаДобавить, ПрефиксУровня0, Уровня0Индекс);
		Результат.Префикс   	= ПрефиксУровня0;
		Результат.ИндексНового 	= Уровня0Индекс;
	ИначеЕсли Уровень = 1 Тогда
		ИмяКнопкиТекОбласти		= СтрШаблон("%1%2_%3_%4", СсылкаДобавить, ПрефиксУровня1, Уровня0Индекс, Уровня1Индекс);
		Результат.Префикс   	= ПрефиксУровня1;
		Результат.ИндексНового 	= Уровня1Индекс;
	ИначеЕсли Уровень = 2 Тогда
		ИмяКнопкиТекОбласти		= СтрШаблон("%1%2_%3_%4", СсылкаДобавить, ПрефиксУровня2, Уровня0Индекс, Уровня2Индекс);
		Результат.Префикс   	= ПрефиксУровня2;
		Результат.ИндексНового 	= Уровня2Индекс;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Результат.Имя				= СтрЗаменить(ИмяКнопкиТекОбласти, СсылкаДобавить, "");
	Результат.Номер				= Число(СтрЗаменить(Результат.Префикс, "П", ""));
	Результат.НаходитсяВСтроке 	= Уровня0Индекс;
	Результат.Уровень			= Уровень;
	
#Область ПропускПервойСтроки_КотораяВсегдаПрисутствуетВМакетеРегОтчета
    // Просто строка создана по умолчанию при создании отчета

	// Первая строка есть всегда!
	Если Уровень = 0 И Уровня0Индекс = 1 И ИтераторЗаписейТЗПоЗначениюУровеня0 = 0 И ИтераторСамогоВерхнегоСлоя = 0 Тогда
		Возврат Результат;
	КонецЕсли;

	// Первая строка всегда содержит по одной подчиненной (ступенька)!
	Если Уровень > 0 И ИтераторЗаписейТЗПоЗначениюУровеня0 = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	
#КонецОбласти	
	
	// Установить фокус на ячейку -> Устанавливаем область Элементу, а получаем область у Реквизита формы!!!
	Элементы.ТабличныйДокумент.ТекущаяОбласть = ЭтаФорма.ТабличныйДокумент.Область(ИмяКнопкиТекОбласти);		
	ВставитьДополнительнуюСтроку(Ложь);
	
	Результат.ИндексНового = Результат.ИндексНового + 1;
...
&НаСервере
Функция НайтиКрайнююОбластьДанных(ТабДокОбласти, ИмяКнопки, ПрефиксУровня0, УровняN = Неопределено)
	
	Результат 		= Неопределено;
	НайденаОбласть 	= Ложь;
	ИндексСтроки	= 0;
	
	Пока ИндексСтроки < ТабДокОбласти.Количество() Цикл
		
		Если УровняN = Неопределено Тогда
			ИмяИскомойОбласти	= СтрШаблон("%1%2_%3", ИмяКнопки, ПрефиксУровня0, ИндексСтроки);
		Иначе
			ИмяИскомойОбласти	= СтрШаблон("%1%2_%3_%4", ИмяКнопки, УровняN.Префикс, УровняN.НаходитсяВСтроке, ИндексСтроки);
		КонецЕсли;
		НайденаОбласть		= ТабДокОбласти.Найти(ИмяИскомойОбласти);
		
		Если НайденаОбласть <> Неопределено Тогда
...
&НаСервере
Процедура ЗаполнитьСтрокуТабличногоДокумента(_ТабДокОбласти, Источник, ОписаниеДобавления, _ПравилаГруппировки)

	_ПрефиксКолонок = Получить_ПрефиксКолонокТЗ();
	
	_ИтераторПравил = 0;
	Пока _ИтераторПравил < _ПравилаГруппировки.Количество() Цикл
		
		Если ОписаниеДобавления.Уровень <> _ИтераторПравил Тогда
			_ИтераторПравил = _ИтераторПравил + 1;
			Продолжить;
		КонецЕсли;
		
		_ПравилоУровней = _ПравилаГруппировки.Получить(_ИтераторПравил);
		
		Если _ПравилоУровней = Неопределено Тогда
			_ИтераторПравил = _ИтераторПравил + 1;
			Продолжить;
		КонецЕсли;
		
		// Так как в отчете заголовок данных (нумерация колонок) начинается с 4
		ИтераторДанных = _ПравилоУровней.С;
		Пока ИтераторДанных <= _ПравилоУровней.ПО Цикл
			
			_ИмяКолонки = СтрШаблон("%1%2", _ПрефиксКолонок, ИтераторДанных);
			
			
			
#Область ЗаполнениеОбластиТабличногоДокументе_
        _ИмяДобавленнойОбласти = Неопределено;
		// У макет рег. отчета (Раздел "Реестр" - > "Стр. 1") ячейки имеют фиксированную длину, Имя в формате:
		// "П + 1000000 + ИндексКолонки + "_" + ИндексСтроки + "_" + ИндексВложеннойСтроки
		// Ступенька 1 (колонки с 12 по 14) начинаются с 1100000
		// Ступенька 2 (колонки с 15 по 26) начинаются с 1200000
		// У каждой ступеньки есть свой набор строк (вложенные строки индексируются по "ИндексВложеннойСтроки")
		_НомерОбласти = Формат(ОписаниеДобавления.Номер * 100 + ИтераторДанных, "ЧРД=; ЧРГ=; ЧГ=0");
		Если ОписаниеДобавления.Уровень = 0 Тогда
			_ИмяДобавленнойОбласти = СтрШаблон("П%1_%2", 	_НомерОбласти, ОписаниеДобавления.НаходитсяВСтроке);
		Иначе
			_ИмяДобавленнойОбласти = СтрШаблон("П%1_%2_%3", _НомерОбласти, ОписаниеДобавления.НаходитсяВСтроке, ОписаниеДобавления.ИндексНового);
		КонецЕсли;
		
		
		Если  _ИмяДобавленнойОбласти = Неопределено Тогда
			ИтераторДанных = ИтераторДанных + 1;
			Продолжить;
		КонецЕсли;
		_ТабДокОбласти.Области[_ИмяДобавленнойОбласти].Значение = Источник[_ИмяКолонки];
#КонецОбласти
...
#Область _ТестовыеДанные

&НаСервере
Процедура ДобавитьТЕСТОВЫЕДанныеВРазделеОтчета_Реестр_НеСервере(_ПравилаГруппировки)
	
	табТестовыхДанных = ПолучитьТестовыйНаборДанныхНаСервере(_ПравилаГруппировки);
	ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере(табТестовыхДанных, _ПравилаГруппировки);
		
КонецПроцедуры

&НаСервере
Функция ПолучитьТестовыйНаборДанныхНаСервере(_ПравилаГруппировки)
	
	_ТабЗначений 	= Новый ТаблицаЗначений;
	_ИмяКолонок		= "TableColumn";
	_ИмяУровней		= "GroupLevel";
	_ИменяКолонокТЗ	= Новый Массив;
	
	ИтераторГенератораКолонок = 4;
	Пока ИтераторГенератораКолонок < 27 Цикл
		_ИмяКолонки = СтрШаблон("%1%2", _ИмяКолонок, ИтераторГенератораКолонок);
		_ТабЗначений.Колонки.Добавить(_ИмяКолонки, Новый ОписаниеТипов("Строка"), ИтераторГенератораКолонок);
		
		_ИменяКолонокТЗ.Добавить(_ИмяКолонки);
		
		ИтераторГенератораКолонок = ИтераторГенератораКолонок + 1;
	КонецЦикла;
		
	ИтераторГенератораУровняГруппировок = 0;
	Пока ИтераторГенератораУровняГруппировок < 3 Цикл
		_ИмяУровня = СтрШаблон("%1%2", _ИмяУровней, ИтераторГенератораУровняГруппировок);		
		_ТабЗначений.Колонки.Добавить(_ИмяУровня, Новый ОписаниеТипов("Строка"), ИтераторГенератораУровняГруппировок);
		
		_ИменяКолонокТЗ.Добавить(_ИмяУровня);
		
		ИтераторГенератораУровняГруппировок = ИтераторГенератораУровняГруппировок + 1;
	КонецЦикла;
	
	ИтераторГенератораДанных = 0;
	Пока ИтераторГенератораДанных < 6 Цикл
		_ТекЗнчКолонки4 = СтрШаблон("AllTheSame Test%1", ИтераторГенератораДанных);
		строкаТЗ = _ТабЗначений.Добавить();
		
		// Заполняем данные
		Для Каждого _Имя Из _ИменяКолонокТЗ Цикл
			строкаТЗ[_Имя] = _ТекЗнчКолонки4;
		КонецЦикла;       
		
		
		// Так как существующие правила группируют: 1-11, 12-14, 15-26. Пробуем:
		строкаТЗ[_ИмяКолонок + "12"] = "202" + Строка(ИтераторГенератораДанных);
		строкаТЗ[_ИмяКолонок + "13"] = строкаТЗ[_ИмяКолонок + "12"];
		строкаТЗ[_ИмяКолонок + "14"] = строкаТЗ[_ИмяКолонок + "12"];
		
		
		// Заполнения Уровней по правилу группировки >
		Если _ПравилаГруппировки <> Неопределено Тогда
			...
			Пока ИтераторПравилГруппировки < _ПравилаГруппировки.Количество() Цикл					
				...
				Для Каждого _КолонкаТЗ Из _ИменяКолонокТЗ Цикл
					
					Если НЕ СтрНайти(_КолонкаТЗ, _ИмяКолонок) Тогда
						Продолжить;
					КонецЕсли;
					
					_текКлюч = Число(СтрЗаменить(_КолонкаТЗ, _ИмяКолонок, ""));
					Если  _текКлюч >= _ЗнчУровня.С И _текКлюч <= _ЗнчУровня.ПО Тогда
						_ИмяТекУровня 	= СтрШаблон("%1%2", _ИмяУровней, _ЗнчУровня.Уровень);
						_знчСтроки		= строкаТЗ[_КолонкаТЗ];
						строкаТЗ[_ИмяТекУровня] = строкаТЗ[_ИмяТекУровня] + ВРег( СтрЗаменить( СокрЛП(_знчСтроки), " ", "") );
					КонецЕсли;
		...
		ИтераторГенератораДанных = ИтераторГенератораДанных + 1;
	КонецЦикла;
...
#Область РассмотретьКакКУдалению1

//&НаСервере
//Функция ПреобразоватьМассивСоответствийВТаблицуЗначений(мСоответствий)
//	
//	// мСоответствий
//	_ТабЗначений = Новый ТаблицаЗначений;
//	_ТабЗначений.Колонки.Добавить("НомерСтроки", Новый ОписаниеТипов("Строка"));
//	
//	Возврат _ТабЗначений;
//КонецФункции


// Функция - Группировать данные по колонкам
//
// ВАЖНО: Ключ колони должен быть в формате имя ячейки макета рег. отчета
// У заполняемого макет отчета (Реестр - > Стр. 1) ячейки имеют фиксированную длину и имя в формате
// "П + 1000000 + ИндексКолонки + "_" + ИндексСтроки + "_" + ИндексВложеннойСтроки
// Ступенька (колонки с 12 по 14) начинаются с 1100000
// Ступенька (колонки с 15 по 26) начинаются с 1200000
// У каждой ступеньки есть свой набор строк (равен значению "ИндексСтроки")
// это возможно за счет "ступеньки" (увеличения ИндексаКолонки на 100000)
//
// Параметры:
//  мДанных	- Массив ИЗ Соответствие
//	*Индекс массива 	- Это строка отчета
//	*Ключ соответствия 	- Это номер колонки отчета
//
//  ПравилаГруппировки	 - Структура - см. ПравилаГруппировкиРазделаОтчета_Реестр,
//	описывает в какую группу, какой диапазон колонок входит
// 
// Возвращаемое значение:
//  Массив ИЗ
//	*Уровень - Число - уровень группировки
//	*Колонки - Массив ИЗ Структура где Ключ это имя ячейки табличного документа
//	*Вложения - Массив - такой же массив только это данный входящие в группировку
//	*Сравнить - Строка - Значения данной колонки склеены в строку без пробелов и вверхнем регистре
//
//&НаСервере
//Функция ГруппироватьДанныеПоКолонкам(мДанных, ПравилаГруппировки)

//	//	Набор = [
//	//	Уровень
//	//	, ДиапазонКолонокСОдинаковымиЗнч_A
//	//	, [
//	// 		[Уровень, ДиапазонКолонокСОдинаковымиЗнч_B, [... ДиапазонКолонокСОдинаковымиЗнч_C, ЗнчКолонокОднойСтрокой], ЗнчКолонокОднойСтрокой]
//	//	]
//	//	, ЗнчКолонокОднойСтрокой ]
//	
//	_мГруппированныхДанных = Новый Массив;	
//	
//	Для Каждого СтрокаКолонок Из мДанных Цикл
//		_Набор = НаборКакГруппировкаДанныхПоКолонкам();
//		_Набор.Уровень = 0;                 
//		
//		м = Новый Массив;
//		с = Новый Структура;
//		с.Вставить("П1000004_1", СтрокаКолонок.Получить("4"));
//		м.Добавить(с);
//		_Набор.Колонки = м;
//		
//		_мГруппированныхДанных.Добавить(_Набор);
//	КонецЦикла;	
//	
//	Возврат _мГруппированныхДанных;
//	
//КонецФункции

//&НаСервере
//Функция НаборКакГруппировкаДанныхПоКолонкам()

//	//	Набор = 
//	//	Уровень
//	//	, ДиапазонКолонокСОдинаковымиЗнч_A
//	//	, [... ДиапазонКолонокСОдинаковымиЗнч_В, ЗнчКолонокОднойСтрокой]
//	//	, ЗнчКолонокОднойСтрокой

//	_Набор = Новый Структура;
//	_Набор.Вставить("Уровень", 	Неопределено);
//	_Набор.Вставить("Колонки", 	Новый Массив);
//	_Набор.Вставить("Вложения", Новый Массив);
//	_Набор.Вставить("Сравнить", Неопределено);
//	
//	Возврат _Набор;
//	
//КонецФункции

// Процедура - Наполнить таб док данными группировки
//
// Параметры:
//  ТабДок					 - Табличный документ - таб. док. рег. отчета который надо наполнить.
//  мГруппированныхДанных	 - Массив ИЗ
//	*Уровень - Число - уровень группировки
//	*Колонки - Массив ИЗ Структура где Ключ это имя ячейки табличного документа
//	*Вложения - Массив - массив аналогичный "мГруппированныхДанных" только это данный входящие в группировку
//	*Сравнить - Строка - Значения данной колонки склеены в строку без пробелов и вверхнем регистре
//
//&НаСервере
//Процедура НаполнитьТабДокДаннымиГруппировки(ТабДок, мГруппированныхДанных)
//	
//	_ТДОбласти = ТабДок.Области;
//	
//	Для Каждого Набор Из мГруппированныхДанных Цикл
//		
//		Если Набор.Колонки = Неопределено Тогда
//			Возврат;
//		КонецЕсли;
//		
//		Если Набор.Уровень = 0 Тогда
//			// Учесть что одна строка всегда есть!
//			// ВставитьДополнительнуюСтроку();
//		ИначеЕсли Набор.Уровень = 1 Тогда
//			// ВставитьДополнительнуюСтроку();
//		ИначеЕсли Набор.Уровень = 2 Тогда
//			// ВставитьДополнительнуюСтроку();
//		КонецЕсли;		
//		
//		Для Каждого Колонка Из Набор.Колонки Цикл
//			ЗаполнитьЗначенияСвойств(_ТДОбласти, Колонка);
//		КонецЦикла;
//		
//		Если Набор.Вложения.Количество() Тогда
//			НаполнитьТабДокДаннымиГруппировки(ТабДок, Набор.Вложения);
//		КонецЕсли;
//		
//	КонецЦикла;
//		
...
#Область РефаклторКода_ВозможноЭтоНеНужнаяЧастьКода
// Процедура - Заполнить раздел отчета "Реестр" (ВХОДНЫЕ ДАННЫЕ МОЖНО НЕ ГРУППИРОВАТЬ!)
//
// ВНИМАНИЕ: ВХОДНЫЕ ДАННЫЕ МОЖНО НЕ ГРУППИРОВАТЬ, процедура выполняет группировку в соответствии с бланком отчета!
//
// Параметры:
//  мДанных	 - Массив ИЗ Соответствие
//	*Индекс массива - Это строка отчета
//	*Ключ соответствия - Это номер колонки отчета
//
//	Комментарий: Разделение на перебор по "НомерСтрокиОтчета" и "ИтераторДанных"
//	позволит дополнять строки отчета, а не очищать текущее наполнение и заполнять с первой строки.
//
//&НаСервере
//Процедура ДобавитьСтрокуВРазделОтчетаРеестр(мДанных, ИндексДанных, УровеньВСтроке)
//		
//	ОписаниеУровнейВСтроке	= ПравилаГруппировкиРазделаОтчета_Реестр();
//	ОписаниеКолонок			= ОписаниеУровнейВСтроке.Получить(УровеньВСтроке);
//	ИндексСледСтрокиДанных	= ИндексДанных + 1;
//	
//	// Так как Первая строка всегда существует
//	Если ИндексДанных = 0 И УровеньВСтроке = 0 Тогда
//		ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестрНаСервере(мДанных, ОписаниеКолонок);
//	КонецЕсли;
//	
//	Если ОписаниеКолонок = Неопределено ИЛИ ИндексДанных >= мДанных.Количество() Тогда
//		Возврат;
//	КонецЕсли;
//	
//	ИндексТекСтроки  	= ИндексДанных;
//	ИндексСлдСтроки 	= ?(ИндексДанных + 1 < мДанных.Количество(), ИндексДанных + 1, Неопределено);
//	
//	ТекущаяКолонка   	= ДанныеСтрокиВКолонках(мДанных, ИндексТекСтроки, ОписаниеКолонок);
//	
//	Если ИндексСлдСтроки <> Неопределено Тогда
//		СледующаяКолонка   	= ДанныеСтрокиВКолонках(мДанных, ИндексСлдСтроки, ОписаниеКолонок);
//		
//		// ************************************************************************************* ПЕРЕХОД НА СЛЕДУЮЩИЙ УРОВЕНЬ >
//		Если ТекущаяКолонка.КакСтрока = СледующаяКолонка.КакСтрока Тогда
//			
//			ОписаниеКолонок	= ОписаниеУровнейВСтроке.Получить(ОписаниеКолонок.Уровень + 1);					
//			Если ОписаниеКолонок = Неопределено ИЛИ СледующаяКолонка = Неопределено Тогда
//				Возврат;
//			КонецЕсли;
//			
//			ИндексСледСтрокиДанных = ИндексДанных + 1;
//			ДобавитьСтрокуВРазделОтчетаРеестр(СледующаяКолонка.Данные, ИндексСледСтрокиДанных, ОписаниеКолонок.Уровень);
//			
//			Возврат;
//		КонецЕсли;
//		// ************************************************************************************* < ПЕРЕХОД НА СЛЕДУЮЩИЙ УРОВЕНЬ		
...
//	КонецЕсли;
//	
//	Если ОписаниеКолонок.Уровень = 0 Тогда
//		ВставитьДополнительнуюСтроку();
//		Сообщить("Уровень0");
//	ИначеЕсли ОписаниеКолонок.Уровень = 1 Тогда
//		// ВставитьДополнительнуюСтроку();
//		Сообщить("Уровень1");
//		Возврат;
//	ИначеЕсли ОписаниеКолонок.Уровень = 2 Тогда
//		// ВставитьДополнительнуюСтроку();
//		Сообщить("Уровень2");
//		Возврат;
//	КонецЕсли;
//	
//	ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестрНаСервере(ТекущаяКолонка.Данные, ОписаниеКолонок);
//	
//	// РЕКУРСИЯ >				
//	ДобавитьСтрокуВРазделОтчетаРеестр(ТекущаяКолонка.Данные, ИндексСледСтрокиДанных, ОписаниеКолонок.Уровень);
//	// < РЕКУРСИЯ
	

#Область Удалить1	
	//ИтераторДанных = ИндексДанных;
	//Пока ИтераторДанных < мДанных.Количество() Цикл
	//	
	//	ИндексТекСтроки  	= ИтераторДанных;
	//	ИндексСлдСтроки 	= ?(ИтераторДанных + 1 < мДанных.Количество(), ИтераторДанных + 1, Неопределено);
	//	
	//	ТекущаяКолонка   	= ДанныеСтрокиВКолонках(мДанных, ИндексТекСтроки, ОписаниеКолонок);
	//	
	//	Если ИндексСлдСтроки <> Неопределено Тогда
	...
	//			
	//			ИтераторДанных = ИтераторДанных + 1;
	//			ДобавитьСтрокуВРазделОтчетаРеестр(СледующаяКолонка.Данные, ИтераторДанных, ОписаниеКолонок.Уровень);
	//			
	//			Возврат;
	//		КонецЕсли;
	//		// ************************************************************************************* < ПЕРЕХОД НА СЛЕДУЮЩИЙ УРОВЕНЬ		
	...
	//	
	//	
	//	// УровеньВСтроке	= ?(ОписаниеКолонок.Уровень - 1 > 0, ОписаниеКолонок.Уровень - 1, 0); Откатить уровень
	...
	//	
	//	// РЕКУРСИЯ >				
	//	ДобавитьСтрокуВРазделОтчетаРеестр(ТекущаяКолонка.Данные, ИтераторДанных, ОписаниеКолонок.Уровень);
	//	// < РЕКУРСИЯ
	//	
	//	ИтераторДанных = ИтераторДанных + 1;
	//КонецЦикла;
...
//&НаСервере
//Функция ДанныеСтрокиВКолонках(СтрокаДанных, ИндексТекСтроки, ОписаниеКолонок)
//	
//	КолонкаОтчета = Новый Структура;
//	КолонкаОтчета.Вставить("Данные");
//	КолонкаОтчета.Вставить("Значения");
//	КолонкаОтчета.Вставить("КакСтрока");
//	
//	мДанные 	= Новый Массив;
//	мЗначений 	= Новый Массив;
//	мЗнчКакСтр 	= "";
//	
//#Область ПереборДанных
//	
//	ИтераторКолонок = ОписаниеКолонок.С;

//	Пока ИтераторКолонок < ОписаниеКолонок.ПО Цикл
//		Колонка = СтрокаДанных[ИндексТекСтроки].Получить(ИтераторКолонок);
//		
//		Если Колонка <> Неопределено Тогда
//			мДанные.Добавить(Колонка);
//			мЗначений.Добавить(Колонка.Значение);
//			мЗнчКакСтр = мЗнчКакСтр + ВРег(СокрЛП(Строка(Колонка.Значение)));
//		КонецЕсли;
//		
...
//#КонецОбласти
//	
//	КолонкаОтчета.Данные 	= мДанные;
//	КолонкаОтчета.Значения 	= мЗначений;
//	КолонкаОтчета.КакСтрока	= мЗнчКакСтр;
//	
//	Возврат КолонкаОтчета;
//	
//КонецФункции
...
//
&НаСервере
Процедура ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестрНаСервере(мДанных, ОписаниеКолонок)
	
	ТДОбласти = ЭтаФорма.ТабличныйДокумент.Области;	
	// 16 - это знч при создании отчета (создается с одной пустой строкой)
	// делить на 5 так как каждая добавляемая строка занимает 5
	НомерСтрокиОтчета	= ((ТДОбласти.ОсновнаяЧасть2.Верх - 16) / 5) + 1 
	+ ?(ТДОбласти.ОсновнаяЧасть2.Верх > 16, 1, 0);
	
    // *** ГРУППИРОВКА ДАННЫХ >
	//мОдинаковыхЗнчКолонок 				= Новый Массив;
	//ЗнчКолонкиПредыдущейИТекущейСтроки	= Ложь;
	// *** < ГРУППИРОВКА ДАННЫХ
	...
	Пока ИтераторДанных < мДанных.Количество() Цикл		
		...
		// Первая строка есть всегда!
		//Если ТДОбласти.ОсновнаяЧасть2.Верх <> 16 ИЛИ ИтераторДанных > 0 Тогда
		//	ВставитьДополнительнуюСтроку();
		//КонецЕсли;
		
		// ***************
		//Если мОдинаковыхЗнчКолонок.Количество() = 12 Тогда
		//// Добавить строку в ступеньку1
		//			
		//			
		//			
		//	мОдинаковыхЗнчКолонок.Очистить();
		//	Возврат;
		//	
		//ИначеЕсли ТДОбласти.ОсновнаяЧасть2.Верх <> 16 ИЛИ ИтераторДанных > 0 Тогда
		//	
		//	ВставитьДополнительнуюСтроку();
		//	
		//КонецЕсли;
		...
		// У каждой ступеньки есть свой набор строк (равен значению "ИндексСтроки")
		// это возможно за счет "ступеньки" (увеличения ИндексаКолонки на 100000)
		// ИмяКолонкиВФорматеОтчета >
		ПерваяКолонкаПодНомером		= 4;
		НомерКолонкиВФорматеОтчета	= 1000000;
		НомерКолонкиВФорматеОтчета	= НомерКолонкиВФорматеОтчета + ПерваяКолонкаПодНомером;
		// < ИмяКолонкиВФорматеОтчета
		...
		//мПредыдущиеДанные	= Новый Массив;
		//мТекущиеДанные		= Новый Массив;
		
		// Первая колонка в шаблоне отчета начинается с №4
		ИтераторКолонок 	= 4;
		
		
		Пока ИтераторКолонок < мДанных[ИтераторДанных].Количество() И ИтераторКолонок <= ОписаниеКолонок.ПО Цикл			
			
			// ---------------------------------------------------- Ступеньки >
			УровеньСтроки = ПостфиксУровень0;
			Если ОписаниеКолонок.Уровень = 1 Тогда
				УровеньСтроки	= ПостфиксУровень1;
			КонецЕсли;
			
			Если ОписаниеКолонок.Уровень = 2 Тогда
				УровеньСтроки 	= ПостфиксУровень2;
			КонецЕсли;
			// ---------------------------------------------------- < Ступеньки
						
			//знчТекущейКолонки								= мДанных[ИтераторДанных][ Строка(ИтераторКолонок) ];
			//Если ИтераторДанных > 0 Тогда
			//	знчТекущейКолонкиПредыдущейСтроки			= мДанных[ИтераторДанных - 1][ Строка(ИтераторКолонок) ];
			//	
			//	Если знчТекущейКолонки = знчТекущейКолонкиПредыдущейСтроки Тогда
			//		мОдинаковыхЗнчКолонок.Добавить(Истина);
			//	КонецЕсли;
			//	
			//КонецЕсли;
			...
			// ИмяКолонкиВФорматеОтчета >			
			ТекСтрокаСтупенькаС12По14						= ?(ИтераторКолонок = 12, 100000, 0); // см. коммент в начале процедуры
			ТекСтрокаСтупенькаС15ПоN						= ?(ИтераторКолонок = 15, 100000, 0); // см. коммент в начале процедуры
			НомерКолонкиВФорматеОтчета						= НомерКолонкиВФорматеОтчета + (ТекСтрокаСтупенькаС12По14 + ТекСтрокаСтупенькаС15ПоN);
			
			форматНаименованияКолонки						= СтрШаблон("ЧС=; ЧРГ=' '; ЧГ=0; ЧФ=ПЧ_%1%2", НомерСтрокиОтчета, УровеньСтроки);
			ИмяКолонкиВФорматеОтчета						= Формат(НомерКолонкиВФорматеОтчета, форматНаименованияКолонки);
			
			знчТекущейКолонки								= мДанных[ИтераторДанных][ Строка(ИтераторКолонок) ];
			ТДОбласти[ИмяКолонкиВФорматеОтчета].Значение	= знчТекущейКолонки;			
			
			ШагПеребораКолонок			= 1;
			НомерКолонкиВФорматеОтчета	= НомерКолонкиВФорматеОтчета + ШагПеребораКолонок;	
			// < ИмяКолонкиВФорматеОтчета
