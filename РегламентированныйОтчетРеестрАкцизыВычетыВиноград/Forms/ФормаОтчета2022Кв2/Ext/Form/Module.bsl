...
Функция ПрочитатьДанныеИзФайлаЭкселВТабДок(ТабДок)
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = ""Ошибка чтения данных из файла в табличный документ! Убедитесь что у вас запущен 'Толстый клиент'"" ");
		Сообщение.Сообщить();
...
Процедура ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере(ИсточникДанных, _ПравилаГруппировки)
	...
	_ОбработанныеЗначения		= Новый Массив;
	...
	Для Каждого _СтрокаТЗ Из ИсточникДанных Цикл
		...
		Если _ПравилаГруппировки.Количество() Тогда
			
			_ИмяТекГруппировки 	= СтрШаблон("%1%2", _ПрефиксУровняГруппировкиТЗ, _ПравилаГруппировки[0].Уровень);			
			_ОтборУровней 		= Новый Структура();			
			_ОтборУровней.Вставить(_ИмяТекГруппировки, _СтрокаТЗ[_ИмяТекГруппировки]);
			
			_ДанныеУровня0 = ИсточникДанных.НайтиСтроки(_ОтборУровней);
			
			Если _ОбработанныеЗначения.Найти(_СтрокаТЗ[_ИмяТекГруппировки]) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;			
			
		иначе
			_ДанныеУровня0 = ИсточникДанных;
		КонецЕсли;		
		...
		Пока _ИтераторПравил < _ПравилаГруппировки.Количество() Цикл
			...
			Пока ИтераторУровняN < _ДанныеУровня0.Количество() Цикл
				_ИмяТекГруппировки 	= СтрШаблон("%1%2", _ПрефиксУровняГруппировкиТЗ, _ПравилаГруппировки[_ИтераторПравил].Уровень);
				Если _УжеДобавлен.Найти(_ДанныеУровня0[ИтераторУровняN][_ИмяТекГруппировки]) <> Неопределено Тогда
					ИтераторУровняN = ИтераторУровняN + 1;
					Продолжить;
				КонецЕсли;
				
				_ДобавитьСтрокуВТабличныйДокумент(_ТабличныйДокументРегОтчета, _ПравилаГруппировки[_ИтераторПравил], _ИтераторСтрок, ИтераторУровняN);
				_УжеДобавлен.Добавить(_ДанныеУровня0[ИтераторУровняN][_ИмяТекГруппировки]);
		...
		_ОбработанныеЗначения.Добавить(_СтрокаТЗ[_ИмяТекГруппировки]);
...
&НаСервере
Процедура _ДобавитьСтрокуВТабличныйДокумент(ТабДок, _ПравилоГруппировки, _ИтераторСтрок, ИндексУровняN)
	
	ТабДокОбласти 	= ТабДок.Области;
	ИмяТекОбласти	= Неопределено;
	
	ИндексУровня0 	= _ИтераторСтрок;
	ИндексУровня1	= ИндексУровняN 	+ 1;
	ИндексУровня2	= ИндексУровняN 	+ 1;
	
	Если _ПравилоГруппировки.Уровень = 0 Тогда
		// Первая строка есть всегда!
		Если ТабДокОбласти.ОсновнаяЧасть2.Верх = 16 И _ИтераторСтрок = 0 Тогда
			Возврат;
		КонецЕсли;

		ИмяТекОбласти = СтрШаблон("ДобавитьСтрокуП10000_%1", ИндексУровня0);
	ИначеЕсли _ПравилоГруппировки.Уровень = 1 Тогда
		ИмяТекОбласти	= СтрШаблон("ДобавитьСтрокуП11000_%1_%2", ИндексУровня0 + 1, ИндексУровня1);
	ИначеЕсли _ПравилоГруппировки.Уровень = 2 Тогда
		ИмяТекОбласти	= СтрШаблон("ДобавитьСтрокуП12000_%1_%2", ИндексУровня0 + 1, ИндексУровня2);
	Иначе
		Возврат;
	КонецЕсли;
	...
	//ИмяГруппы = МногострочнаяЧастьТекущейОбласти(ЭтаФорма, ИмяТекОбласти);
	//
	//Если ЗначениеЗаполнено(ИмяГруппы) Тогда
	//	
	//	Элементы.ТабличныйДокументКонтекстноеМенюДобавитьСтроку.Доступность = Истина;
	...
	//	
	//	ТестИмя = ИмяПервойГрафы + Координаты + СтрокаСИндексомПоказателя(ИсхКолСтрок + 1);
	//	Если СтрЧислоВхождений(ИмяТекОбласти, "ДобавитьСтроку") = 0
	//		И ТабличныйДокумент.Области.Найти(ТестИмя) <> Неопределено Тогда
	//		Элементы.ТабличныйДокументКонтекстноеМенюУдалитьСтроку.Доступность = Истина;
	//	КонецЕсли;
	//	
