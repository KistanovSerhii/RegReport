...
//
&НаСервере
Процедура ЗаполнитьВнешнимиДаннымиРазделОтчетаРеестрНаСервере(мДанных)
	...
	Пока ИтераторДанных < мДанных.Количество() Цикл		
		...
		Пока ИтераторКолонок < мДанных[ИтераторДанных].Количество() Цикл			
			
			// ---------------------------------------------------- Ступеньки >
			УровеньСтроки = ПостфиксУровень0;
			Если ИтераторКолонок > 11 И ИтераторКолонок < 15 Тогда
				УровеньСтроки	= ПостфиксУровень1;
			КонецЕсли;
			
			Если ИтераторКолонок > 14 Тогда
			...
			ТДОбласти[ИмяКолонкиВФорматеОтчета].Значение = мДанных[ИтераторДанных][ Строка(ИтераторКолонок) ];
