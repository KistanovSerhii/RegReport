...
Процедура ДобавитьДанныеВРазделеОтчета_Реестр_НеСервере(ИсточникДанных, _ПравилаГруппировки)
	...
	Для Каждого _СтрокаТЗ Из ИсточникДанных Цикл
		...
		_ИмяГруппировки0 	= СтрШаблон("%1%2", _ПрефиксУровняГруппировкиТЗ, _ПравилаГруппировки[0].Уровень);			
		...
		Пока ИтераторЗаписейТЗПоЗначениюУровеня0 < _мВсеЗаписиТЗПоЗначениюУровеня0.Количество() Цикл
			...
			Пока ИтераторУровняN < _ПравилаГруппировки.Количество() Цикл
				...
				ОписаниеДобавления = _ДобавитьСтрокуВТабличныйДокумент(_ТабличныйДокументРегОтчета, _ИтераторСтрок, ИтераторУровняN, ИтераторЗаписейТЗПоЗначениюУровеня0);
				// ЗаполнитьСтрокуТабличногоДокумента();
		...
		_ОбработанныеСтроки.Добавить(_СтрокаТЗ[_ИмяТекГруппировки]);
...
#Область РеализацияДобавленияДанныхВМакетРегОтчета

&НаСервере
Функция _ДобавитьСтрокуВТабличныйДокумент(ТабДок, _ИтераторСтрок, Уровень, ИндексУровня)
	...
	Результат.Вставить("ИндексВладельца");
	...
	ОписаниеУровняN.Вставить("ИндексВладельца");
	...
	ОписаниеУровняN.ИндексВладельца	= Уровня0Индекс;	
	...
	Если Уровня0Индекс = 0 И _ИтераторСтрок = 0 Тогда
		Возврат Неопределено;
	КонецЕсли;	
	...
	Если Уровень = 0 Тогда
		...
		Результат.Индекс 	= Уровня0Индекс;
	ИначеЕсли Уровень = 1 Тогда
		...
		Результат.Индекс 	= Уровня1Индекс;
	ИначеЕсли Уровень = 2 Тогда
		...
		Результат.Индекс 	= Уровня2Индекс;
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Результат.Имя				= СтрЗаменить(ИмяКнопкиТекОбласти, СсылкаДобавить, "");	
	Результат.ИндексВладельца 	= Уровня0Индекс;
...
Функция НайтиКрайнююОбластьДанных(ТабДокОбласти, ИмяКнопки, ПрефиксУровня0, УровняN = Неопределено)
	...
	Пока ИндексСтроки < ТабДокОбласти.Количество() Цикл
		...
		Иначе
			ИмяИскомойОбласти	= СтрШаблон("%1%2_%3_%4", ИмяКнопки, УровняN.Префикс, УровняN.ИндексВладельца, ИндексСтроки);
		КонецЕсли;
